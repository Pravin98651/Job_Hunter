...
Uses LinkedIn's unauthenticated jobs-guest API to paginate listing cards,
then concurrently fetches full job descriptions from each listing URL.

Anti-bot:
- Random User-Agent rotation via get_random_headers()
- Exponential backoff on 429 responses
- Semaphore to limit concurrent description fetches
"""
    ...
    for attempt in range(3):
        try:
            ...
            if resp.status_code == 200:
                ...
                wait = 2 ** attempt
                logger.warning(f"LinkedIn 429 (rate limited) — waiting {wait}s before retry")
                await asyncio.sleep(wait)
                continue
            break
        except (httpx.TimeoutException, httpx.ConnectError) as e:
            logger.debug(f"LinkedIn description fetch network error (attempt {attempt + 1}): {e}")
...
async def scrape_linkedin_jobs(query: str, location: str, max_results: int = 10) -> list[JobListingCreate]:
    ...
    try:
        async with httpx.AsyncClient(timeout=20, follow_redirects=True) as client:
            ...
            for start in range(0, max_results, 25):
                ...
                try:
                    ...
                    if response.status_code != 200:
                        logger.warning(f"LinkedIn search page start={start} returned HTTP {response.status_code}")
                        break
                    ...
                    logger.warning(f"Error fetching LinkedIn search page start={start}: {e}")
                    break
            ...
            logger.info(f"[linkedin] Parsed {len(parsed)} job cards from search results")
            ...
            for r in results:
                if isinstance(r, JobListingCreate):
                    ...
                    logger.warning(f"[linkedin] Error fetching job detail: {r}")
